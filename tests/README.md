...
| `goinggone.e2e.spec.js` | the auction end to end, and the rules that are easy to break silently: the auctioneer's **descending ask** and a lot **passed in** when it never makes its reserve; **rounds** banking net worth back into cash with the podium waiting for the last one; **paddles** (unique, two-digit, drawn, raised when you hold the bid) and the sale called to the number; **splitting the increment** — that he comes down a step, that the phone re-offers at the new ask, and that he then WAITS there rather than running out the same gavel; the **purse bands** — asserting what crosses the WIRE, not what the TV draws; the valuation's totals staying shut until the last lot is valued; the auctioneer's **speech bubble**, which must fill even where the device cannot speak; and the iOS rule that speech is granted only from inside a tap |
| `goinggone-auction.e2e.spec.js` | the auctioneer's **clock** — the thing that decides whether the game is fun. Three players play three real rounds while the host records a transcript (it wraps `openWindow` and `say`), and the assertions are made against what the room actually experienced rather than against the constants: no window under `MIN_DECIDE_MS`, every unanswered ask quicker than the one before, a bid buying back a full window, the close arriving as **two** announced beats, no beat left silent, and a lot nobody wants leaving faster than one that sells. The rules it enforces are written up in `goinggone-rules.md` |
