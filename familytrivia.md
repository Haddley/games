...
Architecture mirrors `letterstorm.html`: single self-contained HTML file, PeerJS P2P
(one host, N guests, optional viewers), host-authoritative state.

---

## What's different from letterstorm.html

| Feature | letterstorm.html | familytrivia.html |
|---|---|---|
...
## Networking (same pattern as letterstorm.html)
...
- One HTML file, no build step; PeerJS + `qrcode-generator` from CDN (same versions as letterstorm)
...
- Reveal bar chart animates with CSS transitions; Lone Wolf triggers confetti burst
  (canvas, reused from letterstorm podium) + `navigator.vibrate` on the winning phone
