...
## Context
Build a multiplayer browser implementation of Call My Bluff, playable on smartphones via a shared URL. Architecture must match `cornerthemarket.html` exactly: single self-contained HTML file, PeerJS P2P networking (one host, N guests, optional viewers), same dark/day design system (JetBrains Mono + Orbitron, CSS custom properties, `body.day` toggle).
...
| File | Role |
|------|------|
| `public/games/cornerthemarket.html` | Source pattern for all architecture |
| `public/games/bluff.html` | **New file to create** |

---

## Architecture (mirror cornerthemarket.html exactly)
...
### Home screen
Same layout as cornerthemarket.html: name input, Create Game / Join Game / Join as Viewer cards.
...
## CSS (on top of cornerthemarket.html design system)

All base variables (`--bg`, `--surface`, `--s2`, `--border`, `--text`, `--muted`, `--accent`, `--ag`, `--danger`, `--dg`, `--success`) and utility classes (`.btn`, `.btn-p`, `.btn-g`, `.btn-d`, `.btn-full`, `.fg`, `.card`, `.hdr`, `.err`, `.spin`, `.chips`, `.chip`) are reused verbatim from cornerthemarket.html.
