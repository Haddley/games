# cornerthemarket.html — Implementation Plan (Party do-over)

The Hasbro trading-pit classic rebuilt on the **letterstorm** architecture: phones are
...
## What's different from the old cornerthemarket.html

| Feature | old cornerthemarket.html (liarsdice pattern) | new cornerthemarket.html (letterstorm pattern) |
|---|---|---|
...
## Networking (same pattern as letterstorm)
...
## Verification
1. `python3 -m http.server 8231` → open `/cornerthemarket.html` in 4+ tabs (host, 2 guests, TV)
...
6. Playwright suite `tests/cornerthemarket.e2e.spec.js` covers the above with a screenshot tour (`pit-*.png`)
---
